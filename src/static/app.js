document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsItems = details.participants.length
          ? details.participants
              .map(
                (participant) =>
                  `<li><span class="participant-email">${participant}</span><button class="participant-remove" type="button" data-activity="${encodeURIComponent(
                    name
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    if (!removeButton.matches || !removeButton.matches("button.participant-remove")) {
      return;
    }
